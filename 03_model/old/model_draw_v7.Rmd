---
title: "Data Challenge competition - Football prediction, DRAW PREDICTION"
...
train_indices <- createDataPartition(data$away_win_flag, p = 0.8, list = FALSE)
...
# glmnet (elastic net penalized) based selection - draw
basic_numeric_vars_set2 <- c( "odds_draw",  # !
                              "away_team_opponents_shot_roll4_sum", # ?  # !
                              "away_team_opponents_shot_roll3_sum", # ?  # !
                            "away_opponents_shotontarget_per_shots_roll1",
                            "home_goal_per_shot_roll4",
                            "home_goal_per_shot_roll1",
                            "away_shotontarget_per_shots_roll4"
                             )
...
basic_numeric_vars_set4 <- c("odds_away_team_win", 
                             ...
                             "odds_home_team_defeat_roll2_mean",
                             "odds_home_team_defeat_roll1_mean", # human input

                             "odds_away_team_defeat_roll4_mean",
                             "odds_away_team_defeat_roll3_mean",
                             "odds_away_team_defeat_roll2_mean",
                             "odds_away_team_defeat_roll1_mean", # human input
...
# best variables in logistic regression
basic_numeric_vars_set5 <- c("odds_home_team_win", "odds_away_team_win", "home_team_expected_point_roll4_sum", "away_team_opponents_shot_roll1_sum", "away_team_expected_point_mean", "away_team_opponents_corner_roll4_sum")
...
```{r}
# basic two odds
predictors_0 <- c(basic_numeric_vars_set0)
...
# best variables in logistic regression
predictors_6 <- c(basic_numeric_vars_set5)
...
h2o.init()
data_train_h2o <- as.h2o(data_train[,-c("away_win_flag", "home_win_flag")])
...
sensitivity_cv <- glm_model[["results"]][["Sens"]]
...
# glmnet (elastic net penalized) based selection is the best
bayesglm_model <- train(draw_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_1), with=FALSE],
                   method = "bayesglm",
                   trControl = train_control)
```
...
sensitivity_cv <- bayesglm_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
tune_grid_glmnet <- expand.grid(
               alpha = 0.02, #seq(from = 0, to = 1, by = 0.1), # 0.9 ridge: 0, lasso: 1, elastic net: [0,1]
               lambda =  0.1 #seq(0.0001, 1, length = 100) # 0.01
)

# xbg feature selection is the best, but glmnet selection is very good too
glmnet_model <- train(draw_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_3), with=FALSE],
                   method = "glmnet",
                   trControl = train_control,
...
sensitivity_cv <- glmnet_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
xgb_model <- train(
  draw_flag ~ .,
  data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_4), with=FALSE],
  tuneLength = 1,
  method = 'xgbTree',
  na.action = na.omit,
...
sensitivity_cv <- xgb_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
#  train the model
set.seed(19900829)
tune_grid_gbm <- expand.grid(n.trees = 200, #c(100, 200), # 200
                        interaction.depth = 5, # c(5, 10), # 5
                        shrinkage = 0.01, #c(0.005, 0.01, 0.1), # 0.01
                        n.minobsinnode = 50) #c(5, 50)) # 50

# glmnet is the best feature selection
gbm_model <- train(draw_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_3
                                                                   ), with=FALSE], 
                   method = "gbm",
                   trControl = train_control,
...
sensitivity_cv <- gbm_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
nb_model <- train(draw_flag ~ .,
                  data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_0), with=FALSE],
                  method = "naive_bayes",
...
sensitivity_cv <- nb_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 0.9999
```
...
tune_grid_rf <- expand.grid(
  ...
  .min.node.size = 1000 #c(5, 500) # 1-2 % of training set
)

# random forest is the best feature selection
rf_model <- train(
  draw_flag ~ .,
  data = data_train[ ,colnames(data_train) %in% c("draw_flag", predictors_5), with=FALSE],
  tuneLength = 1,
  method = 'ranger',
  na.action = na.omit,
  importance = 'permutation',
...
sensitivity_cv <- rf_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
sensitivity_cv <- svm_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 1
```
...
sensitivity_cv <- knn_model[["results"]][["Sens"]]
message(paste0("Sensitivity from CV: ", round(sensitivity_cv, digits = 4)))
# 0.982
```
...
```{r}
h2o.init()
data_train_h2o <- as.h2o(data_train[,-c("home_win_flag", "away_win_flag", "match_id")])
...
prediction_holdout_glm <- prediction(predicted_glm_holdout, data_holdout$draw_flag)
...
prediction_holdout_bayesglm <- prediction(predicted_bayesglm_holdout, data_holdout$draw_flag)
...
prediction_holdout_glmnet <- prediction(predicted_glmnet_holdout, data_holdout$draw_flag)
...
prediction_holdout_rf <- prediction(predicted_rf_holdout, data_holdout$draw_flag)
...
prediction_holdout_xgb <- prediction(predicted_xgb_holdout, data_holdout$draw_flag)
...
prediction_holdout_gbm <- prediction(predicted_gbm_holdout, data_holdout$draw_flag)
...
prediction_holdout_nb <- prediction(predicted_nb_holdout, data_holdout$draw_flag)
...
prediction_holdout_svm <- prediction(predicted_svm_holdout, data_holdout$draw_flag)
...
prediction_holdout_knn <- prediction(predicted_knn_holdout, data_holdout$draw_flag)
...
predcition_lists <-c("GLM" = prediction_holdout_glm@predictions,
                     "BAYESGLM" = prediction_holdout_bayesglm@predictions,
...
prediction_table$ensemble1 = (prediction_table$GLM + prediction_table$BAYESGLM + prediction_table$GLMNET +prediction_table$GBM + prediction_table$XGB + prediction_table$RF + prediction_table$NB + prediction_table$KNN + prediction_table$SVM ) / 8

# ensemble 2: model average - selected models
prediction_table$ensemble2 = (prediction_table$GLM + prediction_table$GBM ) / 2

# odds based
prediction_table$odds_based <- data_holdout$.prob_home_win_odds_based_adj

# ensemble 3: w odds based 
prediction_table$ensemble3 = (prediction_table$GLM + prediction_table$BAYESGLM + prediction_table$GLMNET +prediction_table$GBM + prediction_table$RF + prediction_table$odds_based) / 6


# Results - fact
prediction_table$actual <- data_holdout$draw_flag
```
...
auc_holdout_ensemble3<- auc_roc(preds = prediction_table$ensemble3, actuals = ordered(prediction_table$actual))
...
message(paste0("AUC on holdout set for ensemble3 Model: ", round(auc_holdout_ensemble3, digits = 4)))
...
--------------------------------------
```{r}
saveRDS(rf_model, paste0(output,"draw_model_rf.rda"))
saveRDS(glm_model, paste0(output,"draw_model_glm.rda"))
saveRDS(bayesglm_model, paste0(output,"draw_model_bayesglm.rda"))
saveRDS(glmnet_model, paste0(output,"draw_model_glmnet.rda"))
saveRDS(gbm_model, paste0(output,"draw_model_gbm.rda"))
saveRDS(xgb_model, paste0(output,"draw_model_xgb.rda"))
saveRDS(nb_model, paste0(output,"draw_model_nb.rda"))
saveRDS(knn_model, paste0(output,"draw_model_knn.rda"))
saveRDS(svm_model, paste0(output,"draw_model_svm.rda"))
```

LOAD MODELS
```{r}
rf_model <- readRDS(paste0(output,"draw_model_rf.rda"))
glm_model <- readRDS(paste0(output,"draw_model_glm.rda"))

gbm_model <- readRDS(paste0(output,"draw_model_gbm.rda"))
xgb_model <- readRDS(paste0(output,"draw_model_xgb.rda"))
nb_model <- readRDS(paste0(output,"draw_model_nb.rda"))
knn_model <- readRDS(paste0(output,"draw_model_knn.rda"))
svm_model <- readRDS(paste0(output,"draw_model_svm.rda"))
```
...
```{r}
# AUC on holdout set
# predicted_proba_holdout <- predict(draw_model_rf, 
#                          newdata = data_holdout, 
#                          type = "prob")[["yes"]]
# 
# 
# prediction_holdout_proba <- prediction(predicted_proba_holdout, data_holdout$draw_flag)
...
1. CALIBRATION TEST - Logistic Regression
```{r}
...
actual_vs_predicted_glm<- data.frame(
  ...
  predicted = predicted_glm_holdout
)
...
```
A szimpla logisztikus regresszió jól kalibrált.

2. CALIBRATION TEST - Bayes Logistic Regression
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_baysesglm<- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_bayesglm_holdout
)
...
calibration_logit <- actual_vs_predicted_baysesglm %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_baysesglm[["predicted"]], actual_vs_predicted_baysesglm[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
A Bayes Logreg modell jól kalibrált.

3. CALIBRATION TEST - GLM NET
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_glmnet<- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_glmnet_holdout
)
...
calibration_logit <- actual_vs_predicted_glmnet %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_glmnet[["predicted"]], actual_vs_predicted_glmnet[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Egész jól kalibrált modell a GLM Net.

4. CALIBRATION TEST - XGB
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_xgb<- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_xgb_holdout
)
...
calibration_logit <- actual_vs_predicted_xgb %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_xgb[["predicted"]], actual_vs_predicted_xgb[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nem jól kalibrált a modell a XGB modell, hiába tűnik magasnak a pontossága. Döntés: nem használom.

5. CALIBRATION TEST - GBM
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_gmb<- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_gbm_holdout
)
...
calibration_logit <- actual_vs_predicted_gmb %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_gmb[["predicted"]], actual_vs_predicted_gmb[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Egész jól kalibrált modell a GBM.

6. CALIBRATION TEST - Random Forest
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_rf <- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_rf_holdout
)
...
calibration_logit <- actual_vs_predicted_rf %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_rf[["predicted"]], actual_vs_predicted_rf[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nagyon jól kalibrált modell a Random Forest.

7. CALIBRATION TEST - SVM
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_svm <- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_svm_holdout
)
...
calibration_logit <- actual_vs_predicted_svm %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_svm[["predicted"]], actual_vs_predicted_svm[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nagyon rosszul kalibrált az SVM modell.  Döntés: nem használom.


8. CALIBRATION TEST - Naive Bayes
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_nb <- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_nb_holdout
)
...
calibration_logit <- actual_vs_predicted_nb %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_nb[["predicted"]], actual_vs_predicted_nb[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nagyon rosszul kalibrált a Naive Bayes modell.  Döntés: nem használom.


9. CALIBRATION TEST - KNN
```{r}
# calibration plot: how well do estimated vs actual event probabilities relate to each other?
# -------------------------------------------------
actual_vs_predicted_knn <- data.frame(
  actual = ifelse(data_holdout[["draw_flag"]] == "yes", 1, 0),
  predicted = predicted_knn_holdout
)
...
calibration_logit <- actual_vs_predicted_knn %>% 
  # ntile: assign group ids based on a variable. similar to "cut".
  mutate(predicted_score_group = ntile(predicted, num_groups)) %>% 
  group_by(predicted_score_group) %>% 
  summarise(mean_actual = mean(actual), mean_predicted = mean(predicted), num_obs = n())
...
brier <- RMSE(actual_vs_predicted_knn[["predicted"]], actual_vs_predicted_knn[["actual"]])^2
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nagyon rosszul kalibrált a KNN modell.  Döntés: nem használom.