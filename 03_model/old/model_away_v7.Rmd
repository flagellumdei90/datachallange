---
title: "Data Challenge competition - Football prediction - AWAY WIN PREDICTION"
...
basic_numeric_vars_set2 <- c("odds_away_team_win",  # !
                             ...
                             "away_team_corner_roll4_sum", # human input 
                              "away_team_corner_roll3_sum",
                              "away_team_corner_roll2_sum", # human input 
                             
                            "home_team_opponents_corner_roll2_sum", # ?
                            "away_team_shot_roll2_sum", # human input 
...
basic_numeric_vars_set5 <- c("odds_home_team_win", 
                             "odds_away_team_win", 
                             "odds_draw",
                             "away_team_expected_point_mean", 
...
h2o.init()
data_train_h2o <- as.h2o(data_train[,-c("home_win_flag", "draw_flag")])

# Define the target variable
target <- "away_win_flag"
...
# basic 3 odds variable
glm_model <- train(away_win_flag ~ .,
...
# basic 3 odds variable
bayesglm_model <- train(away_win_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_1), with=FALSE],
                   method = "bayesglm",
                   trControl = train_control)
```
...
tune_grid_glmnet <- expand.grid(
               alpha = 0, #seq(from = 0, to = 1, by = 0.1), #ridge: 0, lasso: 1, elastic net: [0,1]
               lambda = 0.2 #seq(0.0001, 1, length = 100)
)

# xbg feature selection is the best, but glmnet selection is very good too
glmnet_model <- train(away_win_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_1), with=FALSE],
                   method = "glmnet",
                   trControl = train_control,
...
xgb_model <- train(
  away_win_flag ~ .,
  data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_4), with=FALSE],
  tuneLength = 1,
  method = 'xgbTree',
  na.action = na.omit,
...
gbm_model <- train(away_win_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_3
                                                                   ), with=FALSE], 
                   method = "gbm",
                   trControl = train_control,
...
nb_model <- train(away_win_flag ~ .,
                  data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_0), with=FALSE],
                  method = "naive_bayes",
...
tune_grid_rf <- expand.grid(
  ...
  .min.node.size = 1000 #c(5, 500, 1000) # 1-2 % of training set 1000                                               
)

# random forest is the best feature selection
rf_model <- train(
  away_win_flag ~ .,
  data = data_train[ ,colnames(data_train) %in% c("away_win_flag", predictors_5), with=FALSE],
  tuneLength = 1,
  method = 'ranger',
  na.action = na.omit,
  importance = 'permutation',
...
```{r}
h2o.init()
data_train_h2o <- as.h2o(data_train[,-c("home_win_flag", "draw_flag", "match_id")])
...
prediction_holdout_glm <- prediction(predicted_glm_holdout, data_holdout$away_win_flag)
...
prediction_holdout_bayesglm <- prediction(predicted_bayesglm_holdout, data_holdout$away_win_flag)
...
prediction_holdout_glmnet <- prediction(predicted_glmnet_holdout, data_holdout$away_win_flag)
...
prediction_holdout_rf <- prediction(predicted_rf_holdout, data_holdout$away_win_flag)
...
prediction_holdout_xgb <- prediction(predicted_xgb_holdout, data_holdout$away_win_flag)
...
prediction_holdout_gbm <- prediction(predicted_gbm_holdout, data_holdout$away_win_flag)
...
prediction_holdout_nb <- prediction(predicted_nb_holdout, data_holdout$away_win_flag)
...
prediction_holdout_svm <- prediction(predicted_svm_holdout, data_holdout$away_win_flag)
...
prediction_holdout_knn <- prediction(predicted_knn_holdout, data_holdout$away_win_flag)
...
# Results - fact
prediction_table$actual <- data_holdout$away_win_flag
```
...
--------------------------------------
```{r}
saveRDS(rf_model, paste0(output,"away_model_rf.rda"))
saveRDS(glm_model, paste0(output,"away_model_glm.rda"))
saveRDS(bayesglm_model, paste0(output,"away_model_bayesglm.rda"))
saveRDS(glmnet_model, paste0(output,"away_model_glmnet.rda"))
saveRDS(gbm_model, paste0(output,"away_model_gbm.rda"))
saveRDS(xgb_model, paste0(output,"away_model_xgb.rda"))
saveRDS(nb_model, paste0(output,"away_model_nb.rda"))
saveRDS(knn_model, paste0(output,"away_model_knn.rda"))
saveRDS(svm_model, paste0(output,"away_model_svm.rda"))
```

LOAD MODELS
```{r}
rf_model <- readRDS(paste0(output,"away_model_rf.rda"))
away_model_glm <- readRDS(paste0(output,"away_model_glm.rda"))
away_model_gbm <- readRDS(paste0(output,"away_model_gbm.rda"))

xgb_model <- readRDS(paste0(output,"away_model_xgb.rda"))
nb_model <- readRDS(paste0(output,"away_model_nb.rda"))
knn_model <- readRDS(paste0(output,"away_model_knn.rda"))
svm_model <- readRDS(paste0(output,"away_model_svm.rda"))
```
...
prediction_holdout_proba <- prediction(predicted_proba_holdout, data_holdout$away_win_flag)
...
message(paste0("Brier score: ", round(brier, digits = 4)))
```
Nem jól kalibrált modell a GLM Net.
