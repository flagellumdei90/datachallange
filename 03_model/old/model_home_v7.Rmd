...
basic_numeric_vars_set2 <- c("odds_away_team_win",  # !
                             ...
                             "away_team_opponents_corner_roll3_sum", # human input
                              ...
                              "away_team_opponents_shot_roll2_sum", # human input
...
tune_grid_glmnet <- expand.grid(
               alpha = 0, #seq(from = 0, to = 1, by = 0.1), #ridge: 0, lasso: 1, elastic net: [0,1] 0.9
               lambda = 1 #seq(0.0001, 1, length = 100) #0.01
)
...
gbm_model <- train(home_win_flag ~ .,
                   data = data_train[ ,colnames(data_train) %in% c("home_win_flag", predictors_3
                                                                   ), with=FALSE], 
                   method = "gbm",
                   trControl = train_control,
...
```{r}
h2o.init()
data_train_h2o <- as.h2o(data_train[,-c("away_win_flag", "draw_flag", "match_id")])
